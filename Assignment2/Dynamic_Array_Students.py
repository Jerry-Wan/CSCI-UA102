...
class UserDefinedDynamicArray:
    ...
    def reverse(self):
        ...
        # reverse the list
        # your code
        for a in range(int((self._n/2))):
            self._A[a],self._A[int((self._n))-1-a]= self._A[int((self._n))-1-a],self._A[a]
        return self._A
    ...
    def __add__(self,other):
        # Task 6
        # '+' Operator Overloading for UserDefinedDyamicArray Class like myist1+myList2 will return a list containing all the elements of myList1 and then myList2
        # Your code
        a = UserDefinedDynamicArray()
        a.extend(self)
        a.extend(other)
    ...
    def sort(self, order = "asc"):
        ...
        if order == "asc":
            for a in range(1,len(self)):
                while self[a] < self[a - 1] and a > 0:
                    self[a], self[a - 1] = self[a - 1], self[a]
                    a -= 1
        elif order == "desc":
            for b in range(1,len(self)):
                while self[b] > self[b - 1] and b > 0:
                    self[b], self[b - 1] = self[b - 1], self[b]
                    b -= 1
        else:
            pass
