...
class Tree:
    ...
    def preorderPrint(self,node):
        ...
        :return: nothing.
        """
        for i in self._subtree_preorder(node):
    ...
    def postorderPrint(self,node):
        ...
        :return: nothing.
        """
        for i in self._subtree_postorder(node):
            print(i._element, end = " ")
        print()
    ...
    def levelorderPrint(self, node):
        ...
        while(len(result) > 0): 
            print(result[0]._element,end = '') 
            a = result.pop(0) 
  
            if a._left is not None: 
                result.append(a._left) 
  
            if a._right is not None: 
                result.append(a._right)
        print()
    ...
    def flip_tree(self, node = None):
        ...
        if node == None and count != 0:
           ...
           temp = node
           self.flip_tree(node._left,count)      
           self.flip_tree(node._right,count)
           temp = node._left
           node._left = node._right
           node._right = temp
