...
class Tree:
    ...
    def evaluate(self):
        ...
        outList=[]
        queue=[self._root]
        while queue!=[] and self._root:
            outList.append(queue[0]._element)
            if queue[0]._left!=None:
                queue.append(queue[0]._left)
            if queue[0]._right!=None:
        ...
        while len(operator)!=0:
            ...
            del  numbers[len(numbers)-1]
            operator1 = operator[len(operator)-1]
            del operator[len(operator)-1]
            if operator1 == "*":
                ...
                result1 = number2-number1
            if len(operator) <= 1:
                numbers.append(result1)
            else:
                operator2 = operator[len(operator)-1]
                if operator2 == "*" or operator2 == "/":
                    operator3 = operator[len(operator)-2]
                    if operator3 == "+" or operator3 == "-":
                        del operator[len(operator)-1]
                        number3 = numbers[len(numbers)-1]
                        del  numbers[len(numbers)-1]
                        number4 = numbers[len(numbers)-1]
                        del  numbers[len(numbers)-1]
                        if operator2 == "*":
...
def is_isomorphic(tree1, tree2):
    ...
    """
    # Task 4
    return flip(tree1, tree2, tree1._root, tree2._root)

def flip(tree1, tree2, node1, node2):
    ...
    if (node1 == None and node2 != None) or (node1 != None and node2 == None):      
        ...
        return False
    elif node1._left == None and node2._left == None:
        return flip(tree1, tree2, node1._right, node2._right)
    elif (node1._left != None and node2._left != None) and (node1._left._element == node2._left._element):           #If left is true
        return flip(tree1, tree2, node1._left, node2._left) and flip(tree1, tree2, node1._right, node2._right)
    else:
        return flip(tree1, tree2, node1._left, node2._right) and flip(tree1, tree2, node1._right, node2._left)       #Flip 
...
def build_expression_tree(postfix):
    ...
    for s in postfix: 
        if not s.isnumeric(): 
            node = tree.TreeNode(s) 
            pop1 = arr.pop() 
            pop2 = arr.pop() 
            pop1._parent = pop2._parent = node
            node._right = pop1 
            node._left = pop2 
            arr.append(node) 
        else: 
            node = tree.TreeNode(int(s))
            arr.append(node)
    tree._root = arr.pop() 
    return tree
