...
class Tree:
    ...
    def has_duplicate(self):
        ...
        temp = []               #Create a list to check if it's repeated 
        for i in self.nodes():
            if i._element not in temp:
                temp.append(i._element)
            else:
                return True
        return False
    ...
    def is_height_balanced(self):
        ...
        if self.is_empty():
            raise Empty("Tree is empty")
        for i in self.nodes():
            if abs(self.tree_height(i._left) - self.tree_height(i._right)) > 1:     #Return false if difference is greater than 1
                return False
        return True
    ...
    def tree_height(self,node):
        if node == None:
            ...
            return max(self.tree_height(node._left),self.tree_height(node._right)) + 1    #Recursion
    ...
    def evaluate(self):
        ...
        return self.number(self._root)
    
    def number(self, node):
        if node == None:
            return
        if type(node._element) is not str:       #It's a number 
            return node._element
        else:                                    #It's an operator 
            x = self.number(node._left)          #Iterate until find number 
            y = self.number(node._right)
            return self.operators(node._element,x,y)       #Calculate value 
            
    def operators(self,op,x,y):
        if op == "+":
            return x + y
        if op == "-":
            return x - y
...
def is_isomorphic(tree1, tree2):
    ...
    """
    # Task 4
    return flip_tree(tree1, tree2, tree1._root, tree2._root)

def flip_tree(tree1, tree2, node1, node2):
    if node1 == None and node2 == None:          #Stop recurtion
        return True
    if (node1 == None and node2 != None) or (node1 != None and node2 == None):       #Wrong cases
        return False
    elif node1._element != node2._element:
        return False
    elif node1._left == None and node2._left == None:       #Test right child if left is empty
        return flip_tree(tree1, tree2, node1._right, node2._right)
    elif (node1._left != None and node2._left != None) and (node1._left._element == node2._left._element):           #If left is true
        return flip_tree(tree1, tree2, node1._left, node2._left) and flip_tree(tree1, tree2, node1._right, node2._right)
    else:
        return flip_tree(tree1, tree2, node1._left, node2._right) and flip_tree(tree1, tree2, node1._right, node2._left)       #Flip 
...
def build_expression_tree(postfix):
    ...
    list1 = [] 
    for i in postfix: 
        if i in ["+","-","*","/"]:           #If it's operator
            node = tree.TreeNode(i)          #Add node
            right_child = list1.pop()        #Set two children and parent 
            left_child = list1.pop() 
            right_child._parent = node
            left_child._parent = node
            node._right = right_child 
            node._left = left_child 
            list1.append(node)  
        elif i.isnumeric():                  #If it's number
            node = tree.TreeNode(int(i))
            list1.append(node)
    tree._root = list1.pop() 
    return tree
