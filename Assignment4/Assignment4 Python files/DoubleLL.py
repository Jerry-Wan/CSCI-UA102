class DoubleLinkedList:
    ...
    def sameSame(self, otherlist):
        ...
        currentOther = otherlist._head
        while currentSelf._next != None and currentOther._next != None:
            if currentSelf._element != currentOther._element:
                return False
            else:
                currentSelf = currentSelf._next
    ...
    def feed(self, otherlist, n):
        ...
        if otherlist.is_empty():
            raise Empty("second list is empty")
        a = self._head             
        b = self._head._next
        c = otherlist._head._next
        for i in range(n):
            self._insert_between(c._element, a, b)
            a = a._next
            c = c._next
    ...
    def del_anything_occured(self, otherlist):
        ...
        if otherlist.is_empty():
            return
        a = otherlist._head._next      
        while a != otherlist._tail:
            b = self._head._next       
            while b != self._tail:
                if b._element == a._element:        
                    b = b._next
                    num = self._delete_node(b._prev) 
                else:
                    b = b._next
            a = a._next
