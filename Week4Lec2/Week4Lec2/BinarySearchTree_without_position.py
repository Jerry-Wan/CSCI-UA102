...
class BinarySearchTree(Tree):
    ...
    def minimum(self):
        ...
        return self.first()._element #return self.first().element()
    ...
    def second_minimum_without_using_after(self):
        '''
        ...
        self.helper_second_minimum(self._root,c)
        
        return c[1]
        #pass

    def helper_second_minimum(self,node,c):
        '''
        :return: the second smallest element stored within self BST.
        '''
        # Task 3
        ...
        self.helper_second_minimum(node._left,c)
        ...
        self.helper_second_minimum(node._right,c)
        #pass

    def is_valid_without_inorder_traversal(self):
        prev = [None]
        return self.helper_is_valid(self.root(),prev)
    
    def helper_is_valid(self,node,prev):

        if node is None:
            return True

        if (node._left is not None and not self.helper_is_valid(node._left,prev)):
            return False

        if (prev[0] is not None and prev[0]._element >= node._element):# considering distinct keys
            return False

        prev[0] = node

        return self.helper_is_valid(node._right,prev)
